    ...
    signup: (request, response, next) => {
        ...
        bcrypt.hash(request.body.password, 10)
            .then(hash => {
                ...
                let sql ="INSERT INTO mb_user(MUS_NOM, MUS_PRENOM, MUS_LOGIN, MUS_PASSWORD, MUS_FONCTION, MUS_QUICREAT, MUS_DATECREAT) VALUES(?,?,?,?,?,?,?)";
                let values = ['TT','TT',request.body.login,hash,'BACKEND','TEST','2020-03-21'];
                conn.query(sql, values, (err, res) => {
                    if (err || (res.length === 0)) {
                        log(__filename + " login()", "try to get unexcited user");
                        return response.status(401).json({ error: 'error ' + err, message: 'No user !'});
                    } else {
                        log(__filename + " signup()", "user created successfully : " + request.params.login);
